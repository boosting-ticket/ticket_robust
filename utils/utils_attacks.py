...
def madry_train_one_epoch(m, masks, loss_fn, optimizer, loader_train, args, verbose=True):
    model = m.model
    model.train()
    for t, (x, y) in enumerate(loader_train):
        ...
        #print(args.attack_iter, args.epsilon, args.eps_step, args.clip_min, args.clip_max, x.max(), x.min())
        ...
        if (verbose > 0) and (t % verbose == 0):
            ...
            print('Batch [%d/%d] training loss = %.8f, training pgd_acc = %.8f' % (t, len(loader_train), loss.data, acc))
        

def test_madry(model, loader, args, n_steps=0):
    """
