...
def read_adv_acc(file_name, sig, length=5):
	is_valid = False
	is_test = False
...
for i in range(2):
	...
	for j, name in enumerate(names):
		init_acc, init_adv_acc, init_test_acc = read_adv_acc(str(name)+"/init_enhance_madry_m0.01_warmup0.1_nn.log", ACC_SIG)
		seed9_acc, seed9_adv_acc, seed9_test_acc = read_adv_acc(str(name)+"/seed9_enhance_madry_m0.01_warmup0.1_nn.log", ACC_SIG)
		print(init_acc.shape)
		axes[0].plot(range(init_acc.shape[0]), np.array(init_acc), "-", color="C"+str(j), linewidth=2.0, label=name)
		axes[1].plot(range(init_adv_acc.shape[0]), np.array(init_adv_acc), color="C"+str(j), linewidth=2.0, label=name)
		if i==0:
...
plt.tight_layout()
plt.legend(handles=lines, bbox_to_anchor=(-0.9, 1.05, 1.5, -.102), loc="lower left",
	mode="expand", borderaxespad=0., fontsize=15, ncol=6)
plt.savefig("fgsm_step_large.pdf", bbox_inches='tight')
plt.show()
