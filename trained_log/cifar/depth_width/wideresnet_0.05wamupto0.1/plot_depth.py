...
for expand in [False, True]:
	...
	for c, d in enumerate(depths): 
		init_acc, init_test_acc = read_acc("wideresnet_"+str(d)+"_10/init_enhance_m0.05_warmup0.1.log", ACC_SIG)
		seed9_acc, seed9_test_acc = read_acc("wideresnet_"+str(d)+"_10/seed9_enhance_m0.05_warmup0.1.log", ACC_SIG)
		plt.plot(np.arange(len(init_acc)), init_acc, linewidth=2.0, label=str(d))
		#plt.plot(np.arange(len(seed9_acc)), seed9_acc, "--", color="C"+str(c+1), linewidth=2.0, label=str(d)+"rand")
	...
	if expand:
		plt.savefig("prune_depth_acc_expand.pdf")
	else:
		plt.savefig("prune_depth_acc.pdf")
	plt.show()
	figure_num += 1 