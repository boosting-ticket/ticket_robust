...
for i in range(2):
	for w in widths:
		init_acc, init_test_acc = read_acc("wideresnet_34_"+str(w)+"/init_enhance_m0.05_0.01.log", ACC_SIG)
		#seed9_acc, seed9_test_acc = read_acc("wideresnet_34_"+str(w)+"/seed9_enhance_m0.05_warmup0.1.log", ACC_SIG)
		axes[i].plot(np.arange(len(init_acc)), init_acc, linewidth=2.0, label="34-"+str(w))
...
plt.savefig("prune_width_acc.pdf", bbox_inches='tight')
plt.show()
figure_num += 1 